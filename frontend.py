...
def try_stream(graph_app, inputs: Dict[str, Any], config: Dict[str, Any]) -> Iterator[Tuple[str, Any]]:
    """
    Stream graph progress and capture the final state.
    """
    final_state = {}
    try:
        for step in graph_app.stream(inputs, config=config, stream_mode="updates"):
            yield ("updates", step)
            # Keep track of the latest state to return at the end
            if isinstance(step, dict):
                inner = next(iter(step.values())) if len(step) == 1 and isinstance(next(iter(step.values())), dict) else step
...
if run_btn:
    ...
    # Pass only what the core text backend requires
    inputs: Dict[str, Any] = {
        "topic": topic.strip()
    }
    
    # Crucial limit to prevent Groq API rate crashes
    config: Dict[str, Any] = {
        "max_concurrency": 1
    }
